...
exports.shouldInjectProtectiveContext = shouldInjectProtectiveContext;
exports.getContextualPriorities = getContextualPriorities;
function injectEmotion(userInput, context, emotionalState, config = {
    ...
    const enhancedPrompt = constructEnhancedPrompt(userInput, emotionalContext, situationalContext, loyaltyContext, protectiveContext, emotionalState, config);
    return enhancedPrompt;
}
function buildEmotionalContext(state, config) {
    const contextParts = [];
...
function buildSituationalContext(context, state) {
    if (!state.situational_awareness)
        return '';
    const contextParts = [];
...
function buildLoyaltyContext(state, config) {
    if (!config.include_loyalty_context)
        return '';
    const contextParts = [];
...
function buildProtectiveContext(state, context, config) {
    if (!config.include_protective_context)
        return '';
    const contextParts = [];
...
function constructEnhancedPrompt(userInput, emotionalContext, situationalContext, loyaltyContext, protectiveContext, state, config) {
    const promptParts = [];
...
function getResponseGuidelines(state, config) {
    switch (state.primary_emotion) {
        case 'protective':
...
function calculateEmotionalWeight(state) {
...
function shouldInjectProtectiveContext(state, context) {
...
function getContextualPriorities(state) {
    const priorities = [];
    if (state.protective_mode_active) {
...
//# sourceMappingURL=emotion-injector.js.map